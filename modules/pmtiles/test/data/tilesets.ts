...
export const PMTILESETS = [
  ...
  // v3 folder?
].map((tileset) => `@sensat/loaders-gl-pmtiles/test/data/pmtiles-v2/${tileset}`);

export const PMTILESETS_INVALID = ['empty.pmtiles', 'invalid.pmtiles', 'invalid_v4.pmtiles'].map(
  (tileset) => `@sensat/loaders-gl-pmtiles/test/data/${tileset}`
);
...
].map((tileset) => `@sensat/loaders-gl-pmtiles/test/data/pmtiles-v3/${tileset}`);

export const PMTILESETS_VECTOR = ['protomaps(vector)ODbL_firenze.pmtiles'].map(
  (tileset) => `@sensat/loaders-gl-pmtiles/test/data/pmtiles-v3/${tileset}`
);
