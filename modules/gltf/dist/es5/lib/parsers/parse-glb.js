...
function parseGLBSync(glb, arrayBuffer) {
  ...
  var version = dataView.getUint32(byteOffset + 4, LITTLE_ENDIAN);
  var byteLength = dataView.getUint32(byteOffset + 8, LITTLE_ENDIAN);
  Object.assign(glb, {
    header: {
  ...
  switch (glb.version) {
    ...
    default:
      throw new Error("Invalid GLB version ".concat(glb.version, ". Only supports version 1 and 2."));
  }
}
function parseGLBV1(glb, dataView, byteOffset) {
  (0, _loaderUtils.assert)(glb.header.byteLength > GLB_FILE_HEADER_SIZE + GLB_CHUNK_HEADER_SIZE);
  var contentLength = dataView.getUint32(byteOffset + 0, LITTLE_ENDIAN);
  var contentFormat = dataView.getUint32(byteOffset + 4, LITTLE_ENDIAN);
...
function parseGLBChunksSync(glb, dataView, byteOffset, options) {
  while (byteOffset + 8 <= glb.header.byteLength) {
    var chunkLength = dataView.getUint32(byteOffset + 0, LITTLE_ENDIAN);
    var chunkFormat = dataView.getUint32(byteOffset + 4, LITTLE_ENDIAN);
    byteOffset += GLB_CHUNK_HEADER_SIZE;
    switch (chunkFormat) {
