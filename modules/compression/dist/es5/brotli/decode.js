...
var makeBrotliDecode = function makeBrotliDecode() {
  ...
  function initState(s, input) {
    ...
    s.blockTrees[0] = 7;
    s.distRbIdx = 3;
    var maxDistanceAlphabetLimit = calculateDistanceAlphabetLimit(0x7ffffffc, 3, 15 << 3);
  ...
  function decodeMetaBlockLength(s) {
    ...
    if (sizeNibbles == 7) {
      ...
      for (var _i = 0; _i < sizeNibbles; _i++) {
        if (s.bitOffset >= 16) {
          s.accumulator32 = s.shortBuffer[s.halfOffset++] << 16 | s.accumulator32 >>> 16;
          s.bitOffset -= 16;
        }
  ...
  function transformDictionaryWord(dst, dstOffset, src, srcOffset, len, transforms, transformIndex) {
    ...
    if (transformType == 10 || transformType == 11) {
      ...
      while (len > 0) {
        ...
        if (_c < 0x80) {
        ...
        } else if (_c < 0xc0) {} else if (_c < 0xe0) {
          if (len >= 2) {
            var c1 = dst[shiftOffset + 1];
            scalar += c1 & 0x3f | (_c & 0x1f) << 6;
          ...
          if (len >= 3) {
            var _c2 = dst[shiftOffset + 1];
            var c2 = dst[shiftOffset + 2];
            scalar += c2 & 0x3f | (_c2 & 0x3f) << 6 | (_c & 0x0f) << 12;
            dst[shiftOffset] = 0xe0 | scalar >> 12 & 0x0f;
            dst[shiftOffset + 1] = _c2 & 0xc0 | scalar >> 6 & 0x3f;
          ...
          if (len >= 4) {
            var _c3 = dst[shiftOffset + 1];
            var _c4 = dst[shiftOffset + 2];
            var c3 = dst[shiftOffset + 3];
            scalar += c3 & 0x3f | (_c4 & 0x3f) << 6 | (_c3 & 0x3f) << 12 | (_c & 0x07) << 18;
            dst[shiftOffset] = 0xf0 | scalar >> 18 & 0x07;
            dst[shiftOffset + 1] = _c3 & 0xc0 | scalar >> 12 & 0x3f;
            dst[shiftOffset + 2] = _c4 & 0xc0 | scalar >> 6 & 0x3f;
    ...
    for (var _len2 = rootBits + 1, _step = 2; _len2 <= 15; _len2++, _step <<= 1) {
      for (; count[_len2] > 0; count[_len2]--) {
        if ((key & mask) != low) {
          ...
          tableBits = nextTableBitSize(count, _len2, rootBits);
...
exports.BrotliDecode = BrotliDecode;
