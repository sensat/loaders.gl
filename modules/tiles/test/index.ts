// loaders.gl
// SPDX-License-Identifier: MIT
// Copyright (c) vis.gl contributors

import './utils/doubly-linked-list.spec';

import './tileset/tile-3d.spec';
import './tileset/tileset-3d.spec';
import './tileset/tileset-traverser.spec';
import './tileset/grouped-tiles.array.spec';

import './tileset/helpers/get-frame-state.spec';
import './tileset/helpers/zoom.spec';
import './tileset/helpers/bounding-volume.spec';
import './tileset/helpers/tiles-3d-lod.spec';

// I3S Specific tests
// TODO - deck.gl dependency
import './tileset/format-i3s/i3s-lod.spec';
import './tileset/format-i3s/i3s-pending-tiles-register.spec';
