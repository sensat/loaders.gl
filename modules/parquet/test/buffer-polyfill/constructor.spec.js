...
test('new buffer from array', function (t) {
  t.equal(
    new BufferPolyfill([1, 2, 3]).toString(),
    '\u0001\u0002\u0003'
  )
  t.end()
})
...
test('new buffer from array w/ negatives', function (t) {
  t.equal(
    new BufferPolyfill([-1, -2, -3]).toString('hex'),
    'fffefd'
  )
  t.end()
})
...
test('new buffer from array with mixed signed input', function (t) {
  t.equal(
    new BufferPolyfill([-255, 255, -128, 128, 512, -512, 511, -511]).toString('hex'),
    '01ff80800000ff01'
  )
  t.end()
})
...
test('new buffer from string', function (t) {
  t.equal(
    new BufferPolyfill('hey', 'utf8').toString(),
    'hey'
  )
  t.end()
})
...
test('new buffer from buffer', function (t) {
  const b1 = new BufferPolyfill('asdf')
  const b2 = new BufferPolyfill(b1)
...
test('new buffer from ArrayBuffer', function (t) {
  if (typeof ArrayBuffer !== 'undefined') {
    ...
    const b = new BufferPolyfill(arraybuffer)
    t.equal(b.length, 4)
    t.equal(b[0], 0)
    t.equal(b[1], 1)
    t.equal(b[2], 2)
...
test('new buffer from buffer.toJSON() output', function (t) {
  ...
  const buf = new BufferPolyfill('test')
  const json = JSON.stringify(buf)
  const obj = JSON.parse(json)
  const copy = new BufferPolyfill(obj)
