...
test('buffer.toJSON', function (t) {
  const data = [1, 2, 3, 4]
  t.deepEqual(
    new BufferPolyfill(data).toJSON(),
...
test('buffer.copy', function (t) {
  ...
  const buf2 = new BufferPolyfill(26)
...
test('concat() a varying number of buffers', function (t) {
  const zero = []
  const one = [new BufferPolyfill('asdf')]
  const long = []
  for (let i = 0; i < 10; i++) {
    long.push(new BufferPolyfill('asdf'))
  }

  const flatZero = BufferPolyfill.concat(zero)
  const flatOne = BufferPolyfill.concat(one)
  const flatLong = BufferPolyfill.concat(long)
  const flatLongLen = BufferPolyfill.concat(long, 40)
...
test('concat() works on Uint8Array instances', function (t) {
  const result = BufferPolyfill.concat([new Uint8Array([1, 2]), new Uint8Array([3, 4])])
  const expected = BufferPolyfill.from([1, 2, 3, 4])
  t.deepEqual(result, expected)
  t.end()
})

test('concat() works on Uint8Array instances for smaller provided totalLength', function (t) {
  const result = BufferPolyfill.concat([new Uint8Array([1, 2]), new Uint8Array([3, 4])], 3)
  const expected = BufferPolyfill.from([1, 2, 3])
  t.deepEqual(result, expected)
  t.end()
})

test('fill', function (t) {
  const b = new BufferPolyfill(10)
  b.fill(2)
...
test('copy() empty buffer with sourceEnd=0', function (t) {
  const source = new BufferPolyfill([42])
  const destination = new BufferPolyfill([43])
...
test('copy() after slice()', function (t) {
  const source = new BufferPolyfill(200)
  const dest = new BufferPolyfill(200)
  const expected = new BufferPolyfill(200)
  for (let i = 0; i < 200; i++) {
    source[i] = i
    dest[i] = 0
  }
...
test('copy() ascending', function (t) {
  const b = new BufferPolyfill('abcdefghij')
  b.copy(b, 0, 3, 10)
...
test('buffer.slice sets indexes', function (t) {
  t.equal((new BufferPolyfill('hallo')).slice(0, 5).toString(), 'hallo')
  t.end()
})

test('buffer.slice out of range', function (t) {
  t.plan(2)
  t.equal((new BufferPolyfill('hallo')).slice(0, 10).toString(), 'hallo')
  t.equal((new BufferPolyfill('hallo')).slice(10, 2).toString(), '')
  t.end()
})
