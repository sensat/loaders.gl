...
test('ParquetJSONLoader#loader objects', (t) => {
  validateLoader(t, ParquetJSONLoader, 'ParquetJSONLoader');
  validateLoader(t, ParquetJSONWorkerLoader, 'ParquetJSONWorkerLoader');
  t.end();
});
...
test('ParquetJSONLoader#load', async (t) => {
  ...
  if (!isBrowser) {
    ...
    for (const {title, path} of SUPPORTED_FILES) {
      ...
      const table = await load(url, ParquetJSONLoader, {parquet: {url}, worker: true});
