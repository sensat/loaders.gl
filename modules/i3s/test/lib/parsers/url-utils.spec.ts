...
import {
  ...
  // @ts-expect-error
} from '@sensat/loaders-gl-i3s/lib/utils/url-utils';
...
test('i3s-utils#getUrlWithoutParams', async (t) => {
  ...
  const url2 = getUrlWithoutParams('@sensat/loaders-gl-core/test/data/url');
  t.equal(url2, '@sensat/loaders-gl-core/test/data/url');
  t.end();
});
