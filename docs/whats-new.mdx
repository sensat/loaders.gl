...
- **@sensat/loaders-gl-mlt** - New [`MLTLoader`](/docs/modules/mlt/api-reference/mlt-loader) and [`MLTSource`](/docs/modules/mlt/api-reference/mlt-source) add support for the [`MapLibre Tile`](/docs/modules/mlt/formats/mlt) format, with a new [MLT website example](/examples/tiles/mlt).
- **@sensat/loaders-gl-textures** - Texture loaders now return `TextureLevel` objects with explicit `shape`, `format`, and `textureFormat` fields, add manifest-driven [`TextureLoader`](/docs/modules/textures/api-reference/texture-loader), [`TextureArrayLoader`](/docs/modules/textures/api-reference/texture-array-loader), and [`TextureCubeLoader`](/docs/modules/textures/api-reference/texture-cube-loader), and support Radiance HDR textures through [`RadianceHDRLoader`](/docs/modules/textures/api-reference/radiance-hdr-loader).
- **@sensat/loaders-gl-arrow** - Added support for `apache-arrow@21` and a new guide for [Using with Apache Arrow](/docs/developer-guide/apache-arrow).
- **@sensat/loaders-gl-mvt** - New experimental [`MVTWriter`](/docs/modules/mvt/api-reference/mvt-writer) allows GeoJSON to be written in binary MVT tile format.

**Minor Features**

**All modules**

- Pre-bundled workers now exported explicitly in each module's package.json `exports` section.

**@sensat/loaders-gl-core**

- [`LoaderOptions.core`](/docs/modules/core/api-reference/loader-options) - stronger type checks when options are supplied through the new `options.core` option sub-object.

**@sensat/loaders-gl-gltf**

- [`GLTFLoader`](/docs/modules/gltf/api-reference/gltf-loader) supports the release-candidate [`KHR_meshopt_compression`](/docs/modules/gltf/formats/gltf#meshopt-compression) extension, including version 1 attribute streams and the `COLOR` filter, while retaining `EXT_meshopt_compression` support.

**@sensat/loaders-gl-compression**

- `LZOCompression` was removed due to lack of maintenance and licensing concerns with the underlying `lzo-wasm` library.

**@sensat/loaders-gl-shapefile**

- `ShapefileLoader` - Now forwards `dbf` loader options (such as `workerUrl`) when loading sidecar files.

**@sensat/loaders-gl-zip**

- `ZipWriter` adds `options.zip.createFolders` to emit parent directory entries for nested file keys.

**@sensat/loaders-gl-tile-converter**

- `tile-converter` is now broken out into separate "application" and can be versioned and published independently.

**Internal Changes**

- **TypeScript** - TypeScript v5.9, stricter typings, `ArrayBuffer` -> `ArrayBufferLike` codebase migration.
- **Website** - docusaurus@3.9. Website search.
- **Tooling** - Bump to @vis.gl/dev-tools@1.0, GitHub CI optimizations, migrate from volta to corepack
...
- **[`DataSource`](/docs/developer-guide/using-sources) API** Multi-step data loadingi mprovements.
- **Website** - Live examples are now available for more loaders / formats.
- **Examples** - Migrated to React functional components. Minimized boilerplate code.

**@sensat/loaders-gl-core**

- `Loader` type inference improvements: `load()`, `parse()`, `parseSync()` and `parseInBatches()` now infer correct option and return types when an array of loaders is passed.
- `Writer` type inference improvements: `encode()`, `encodeSync()` and `encodeInBatches()` now infer correct option and data types matching the specified writer.
...
- Loader-wide options such as `fetch`, `worker`, `nothrow`, batching limits, etc. are now grouped under `options.core`, while the deprecated top-level aliases continue to work for backwards compatibility.

**@sensat/loaders-gl-mvt**

- [`TableTileSource`](/docs/modules/mvt/api-reference/table-tile-source) - Geospatial tables can now be vector tiled dynamically in the browser.
...
- Removed 'extended SLPK' usage from the converter, `--slpk` is now a default behavior
- Removed `slpk-extractor` tool
- Added option `--egm=None` to disable egm conversion
- `tile-conveter` source code has been converted to TypeScript
...
- `DracoLoader` now ignores unsupported attributes such as FLOAT64, INT64, UINT64.
- `DracoLoader`, `DracoWriter` - Now uses [draco3d `v1.5.7`](https://github.com/google/draco#version-157-release) _decoders_.
...
- New [PMTiles example](/examples/tiles/pmtiles).
...
- `ZipFileSystem` and `IndexedArchive` enable file-by-file reads from large zip archives without loading the entire archive into memory (in Node.js).
- `ZipWriter` adds `options.zip.createFolders` to emit parent directory entries for nested file keys.
...
- Now uses latest [`apache-arrow` v21](/docs/arrowjs/whats-new#v210).
...
_For patch releases, see [CHANGELOG](https://github.com/visgl/loaders.gl/blob/3.4-release/CHANGELOG.md)._
...
**@sensat/loaders-gl-bson** (New module)
...
- [`WMSService`](modules/wms/api-reference/wms-source)
...
- [`CSWService`](modules/wms/api-reference/csw-source)
...
_For patch releases, see [CHANGELOG](https://github.com/visgl/loaders.gl/blob/3.3-release/CHANGELOG.md)._
...
**@sensat/loaders-gl-wms** (New module)
...
**@sensat/loaders-gl-xml** (New module)
...
_For patch releases, see [CHANGELOG](https://github.com/visgl/loaders.gl/blob/3.2-release/CHANGELOG.md)._
...
_For patch releases, see [CHANGELOG](https://github.com/visgl/loaders.gl/blob/3.1-release/CHANGELOG.md)._
...
_For patch releases, see [CHANGELOG](https://github.com/visgl/loaders.gl/blob/3.0-release/CHANGELOG.md)._
...
- TypeScript: many loaders now export a `LoaderOptions` type that can be used to typecheck loader options, e.g `import type {CSVLoaderOptions}` from `@sensat/loaders-gl-csv`.
...
**@sensat/loaders-gl-textures** NEW
...
**@sensat/loaders-gl-tile-converter** NEW
...
**@sensat/loaders-gl-shapefile** NEW
...
**@sensat/loaders-gl-compression** NEW
...
**@sensat/loaders-gl-video** (new loader module)
...
**@sensat/loaders-gl-i3s** (new loader module)
...
**@sensat/loaders-gl-mvt** (new loader module)
...
**@sensat/loaders-gl-terrain** (new loader module)
...
**@sensat/loaders-gl-wkt** (new loader module)
...
- `@sensat/loaders-gl-core`: File Type Auto Detection now supports binary files
- `@sensat/loaders-gl-polyfills`: Fixed `TextEncoder` warnings
- `@sensat/loaders-gl-arrow`: Improved Node 8 support
- `@sensat/loaders-gl-images`: Image file extensions now added to loader object
- `@sensat/loaders-gl-gltf`: Generate default sampler parameters if none provided in gltf file

**@sensat/loaders-gl-3d-tiles** (EXPERIMENTAL)
...
**@sensat/loaders-gl-3d-tiles** (NEW MODULE)

- Support for the 3D tiles format is being developed in the new `@sensat/loaders-gl-3d-tiles` module.
...
**@sensat/loaders-gl-polyfills** (NEW MODULE)

Node support now requires importing `@sensat/loaders-gl-polyfills` before use. This reduces the number of dependencies, bundle size and potential build complications when using other loaders.gl modules when not using Node.js support.

**@sensat/loaders-gl-loader-utils** (NEW MODULE)

Helper functions for loaders have been broken out from `@sensat/loaders-gl-core`. Individual loaders no longer depend on`@sensat/loaders-gl-core` but only on `@sensat/loaders-gl-loader-utils`.
